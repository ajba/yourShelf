...
class WPF_Public {
    ...
    public function __construct($plugin_name, $version) {
		...
		if (!empty($_GET['id']) && !empty($_GET['wpf'])) {
			add_filter('the_content',array($this,'override_wc_filter_query'),1);
		}
    ...
    public function result_page() {
        if (!empty($_POST['id']) && !empty($_POST['wpf'])) {
            $option = WPF_Options::get_option($this->plugin_name, $this->version);
            $forms = $option->get();
        ...
        if ($build) {
			...
			if ( empty($query_args['orderby']) ){
				...
				if ( is_array( $query_args['orderby'] ) ) {
					$query_args['orderby'] = array_merge( array( 'menu_order' => 'ASC' ), $query_args['orderby'] );
				} else {
					$query_args['orderby'] = 'menu_order ' . $query_args['orderby'];
	...
	function change_query( $query ) {
          if ( 'product' === $query->get( 'post_type' ) ) {
			...
			$args = $this->parse_query( $_GET, $forms[ $id ] );
			foreach ( $args as $k => $v ) {
				$query->set( $k, $v );
			}
			remove_action('pre_get_posts',array($this,'change_query'));
		}
	}

      function change_wc_shortcode_query($args){
          $option = WPF_Options::get_option( $this->plugin_name, $this->version );
          $forms = $option->get();
          $id = sanitize_key( $_GET['id'] );
          $q_args = $this->parse_query( $_GET, $forms[ $id ] );
          foreach ( $q_args as $k => $v ) {
              $args[$k]= $v;
          }
          remove_filter('woocommerce_shortcode_products_query',array($this,'change_wc_shortcode_query'));
          return $args;
      }

      function override_wc_filter_query($content){
		  add_action('pre_get_posts',array($this,'change_query'));
		  add_filter('woocommerce_shortcode_products_query',array($this,'change_wc_shortcode_query'));
		  return $content;
      }
}
